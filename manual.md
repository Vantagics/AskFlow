...
服务默认监听 `0.0.0.0:8080`，在浏览器访�?`http://localhost:8080` 即可使用�?

### 1.2 命令行用�?

```
askflow                                              启动 HTTP 服务（默认端�?8080�?
...
使用 `--product` 参数可将导入的文档关联到指定产品。不指定时，文档将导入到公共库。若指定的产�?ID 不存在，系统将报错并中止导入�?
...
API Key 在保存时会自动使�?AES-256-GCM 加密，加密密钥通过环境变量 `ASKFLOW_ENCRYPTION_KEY` 设置。若未设置，系统会自动生成随机密钥�?
...
| `video.keyframe_interval` | 关键帧抽样间隔，默认 10 �?|
| `video.whisper_model` | whisper 模型名称，默�?"base" |
...
仅配�?`ffmpeg_path` 时，系统只提取关键帧进行视觉检索；同时配置 `whisper_path` 后还会进行语音转录，支持基于语音内容的文本检索�?

### 2.5 多模态与检索优化配�?

| 配置�?| 说明 |
|--------|------|
| `embedding.use_multimodal` | 是否启用多模�?Embedding（图片向量化），默认 true |
| `vector.content_priority` | 检索结果排序：`image_text`（默认，优先含图片结果）�?`text_only`（优先纯文本�?|
| `vector.text_match_enabled` | 启用 3 级文本匹配优化，默认 true。通过本地文本匹配和缓存复用减�?API 调用 |
...
视频文件上传后，系统会：
- 调用 ffmpeg 提取音频，再通过 whisper 进行语音转录，转录文本分块后嵌入向量�?
- 调用 ffmpeg 按配置间隔抽取关键帧，通过多模�?Embedding 嵌入向量�?
- �?video_segments 表中记录每个分块对应的视频时间区间，检索时返回精确时间定位

视频功能需要在配置中设�?`video.ffmpeg_path`（必需）和 `video.whisper_path`（可选，不配置则跳过语音转录）�?

系统会自动进行内容去重：
- **文档级去�?*：解析后计算内容 SHA-256 哈希，若已有相同内容的文档则拒绝导入并提示已有文�?ID
- **分块级去�?*：分块后检查数据库中是否已有相同文本的分块，有则直接复用其向量，仅对新文本调用 Embedding API，节�?API 调用
...
返回所有已上传文档及其状态（processing / success / failed）。支持通过 `product_id` 参数筛选特定产品的文档，列表中每个文档会显示所属产品名称或"公共�?标签�?
...
`image_data` 为可选字段，支持传入 base64 格式的图片数据（�?`data:image/png;base64,...`）进行多模态查询。附带图片时，系统会�?
- 跳过意图分类（图片可能包含产品信息）
- 同时进行文本向量检索和图片向量检索（图片使用较低阈�?× 0.6�?
- 合并去重两路检索结�?
- 使用视觉 LLM（GenerateWithImage）结合图片和知识库生成回�?

`product_id` 为可选字段，指定后系统仅在该产品知识库和公共库中检索，不指定则在所有产品中检索�?

### 4.2 回答流程

系统采用多级递进策略处理查询，在保证回答质量的同时尽量减�?API 调用成本�?
...
- 纯文本查询先经过 LLM 意图分类�?
  ...
  - `product`：产品相关问题，进入检索流�?

#### 4.2.2 3 级文本匹配（仅纯文本查询�?

启用 `vector.text_match_enabled` 后，系统在调用完�?RAG 之前先尝试低成本匹配�?

1. **Level 1 �?本地文本匹配（零 API 开销�?*：使用字�?bigram 相似度在分块缓存中搜索，命中且有缓存回答时直接返�?
2. **Level 2 �?向量确认 + 缓存复用（仅 Embedding API�?*：调�?Embedding API 生成向量进行搜索确认，命中且有缓存回答时返回
3. **Level 3 �?完整 RAG（Embedding + LLM�?*：前两级未命中时进入完整流程

#### 4.2.3 向量检�?

- **文本检�?*：将问题向量化，在知识库中检�?Top-K 相关片段
- **图片检�?*（附带图片时）：将图片通过多模�?Embedding 向量化，使用较低阈值（`threshold × 0.6`）进行检索，与文本检索结果合并去�?
- **宽松检�?*：若标准检索无结果，降低阈值重试（接受 score �?0.3 的结果）

#### 4.2.4 结果后处�?

1. **内容优先级排�?*：根�?`vector.content_priority` 配置调整结果顺序（`image_text` 优先含图片结果，`text_only` 优先纯文本结果）
2. **视频时间定位**：查�?`video_segments` 表，为来自视频的分块补充 `start_time` / `end_time` 信息
...
当检索结果来源于视频文档时，`start_time` �?`end_time` 字段包含视频中的时间位置（秒）。对于转录文本分块，这是一个时间区间；对于关键帧，两者相等�?
...
支持格式：jpg、png、gif、webp、bmp。返回图�?URL�?
...
支持�?OAuth 提供商：Google、Apple、Amazon、Facebook。需在配置文件中设置对应�?`client_id`、`client_secret` 等参数�?
...
```
askflow_<模式>_<主机�?_<日期-时间>.tar.gz
askflow_<模式>_<主机�?_<日期-时间>.manifest.json
```

示例�?
- `askflow_full_myserver_20260212-143000.tar.gz` �?全量备份归档
- `askflow_full_myserver_20260212-143000.manifest.json` �?全量备份元数�?
...
基于上次备份�?manifest 文件，按数据级别导出变更内容，避免冗余�?
