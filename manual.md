...
- **多图画廊**：回答中包含多张图片时，以轮播画廊展示，支持前后翻页和圆点导航，点击可全屏查看，避免图片纵向堆叠占用过多空间
- **媒体弹窗播放**：回答中的视频/音频不再内联展示，改为紧凑的播放按钮，点击后弹窗播放，支持时间段跳转
- **引用来源播放**：参考资料列表中的视频/音频旁显示播放按钮（▶️/🎵），点击即可弹窗播放对应媒体
- **流式播放**：视频通过 HTTP Range 请求实现流式播放，无需等待完整下载即可开始观看，媒体文件缓存 1 小时
- **键盘操作**：按 Escape 键可关闭全屏图片和媒体弹窗
