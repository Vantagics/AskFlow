...
(function () {
    ...
    var translations = {
        'zh-CN': {
            ...
            'admin_settings_text_match_hint': '开启后查询按3级处理：1级纯文本匹配（免费）→ 2级向量确认+缓存复用（仅嵌入费用）→ 3级完整RAG（嵌入+LLM费用）',
            ...
            'admin_multimodal_whisper': 'Whisper 配置',
            'admin_multimodal_whisper_path': 'Whisper 路径',
            'admin_multimodal_whisper_path_hint': '留空则使用系统 PATH 中的 whisper，也可填写完整路径',
            'admin_multimodal_whisper_model': 'Whisper 模型',
            'admin_multimodal_whisper_model_hint': '模型越大精度越高但速度越慢，首次使用会自动下载模型文件',
            'admin_multimodal_whisper_install': '📦 Whisper 安装指南',
            ...
            'admin_multimodal_workflow': '上传视频后，系统将自动：1) 使用 FFmpeg 提取音频和关键帧 → 2) 使用 Whisper 将语音转为文字 → 3) 对文字和图像分别生成向量嵌入 → 4) 存入知识库供检索',
        ...
        'en-US': {
            ...
            'admin_multimodal_whisper': 'Whisper Settings',
            'admin_multimodal_whisper_path': 'Whisper Path',
            'admin_multimodal_whisper_path_hint': 'Leave empty to use whisper from system PATH, or enter full path',
            'admin_multimodal_whisper_model': 'Whisper Model',
            'admin_multimodal_whisper_model_hint': 'Larger models are more accurate but slower. Model files are downloaded automatically on first use',
            'admin_multimodal_whisper_install': '📦 Whisper Installation Guide',
            ...
            'admin_multimodal_formats': 'MP4, AVI, MKV, MOV, WebM',
            'admin_multimodal_workflow': 'After uploading a video, the system will: 1) Extract audio and keyframes with FFmpeg → 2) Transcribe speech to text with Whisper → 3) Generate vector embeddings for text and images → 4) Store in knowledge base for retrieval',
